...
object StreamLearn {
  ...
  sealed trait Stream[+A] {
    ...
    def filterSelf(p: A => Boolean): Stream[A] = this match {
      case Empty => empty
      case Cons(h, t) if p(h()) => cons(h(), t().filterSelf(p))
      case Cons(_, t) => t().filterSelf(p)
    }
     ...
     * @param z
     * @param f
     * @tparam B B之前加上了=>，表示B对代码会延迟调用，真正在f中调用了b中的代码时，才执行。而一般的会立即执行B中代码的内容，然后执行f
     * @return
     */
    ...
    def existsNotFold(p: A => Boolean): Boolean = this match {
  ...
  object Stream {
    ...
    */
    ...
    //展开写法,因为传入的是(x,y)，此时p为(x,y)，要把p拆开
    val fibsViaUnfold_1 =
      unfold((0, 1)) { p =>
        p match {
    ...
    //或者这么写
    val fibsViaUnfold_2 =
      unfold((0, 1)) { p => Some(p._1, (p._1, p._1 + p._2)) }
  ...
  def main(args: Array[String]): Unit = {
    //一个代码块为一个返回值，也就是返回值是()=>Int，而不是Int
    val value = Stream(() => {
      println("cjs1");
      1
    }, () => {
    ...
    println(value.take(1).toList(0))

    println(value.filterSelf(_ () < 2))

    //无限流
    val oneList = Stream.ones.take(5).toList
    println(oneList)
